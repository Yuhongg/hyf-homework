...
const oddNumbers = number.filter((number) => number % 2 !== 0);
const newNumbers = oddNumbers.map(function (oddNumbers) {
  const multipliedNumbers = 2 * oddNumbers;
...
//short movie title
const shortMovieTitle = movies.filter(function (movie) {
...
const movieNumber = movies.filter((movie) => {
...
//const taggedMovie = movies.map((rating) => {
//  const newMovie = { ...rating };
//  newMovie.tag = newMovie.rating >= 7 ? "Good" : "Average";
//  return newMovie;
//});

const taggedMovie = movies.map((movie) => ({
  ...movie,
  tag: movie.rating >= 7 ? "Good" : "Bad",
}));

console.log(taggedMovie);
...
const movieRating6 = movies
  .filter((movie) => movie.rating > 6)
  .map((movie) => movie.rating);

console.log(movieRating6);

//Count the total number of movies containing any of following keywords: Surfer, Alien or Benjamin. So if there were 3 movies that contained Surfer, 1 with Alien and

const movieWithKeyWords = movies.reduce((keyWords, movie) => {
  if (
...
//average rating
const averageRating = movies.reduce((sumRating, movie) => {
  movies.forEach((movie) => sumRating + movie.rating);
  return sumRating;
}, {});
// NOthing happened
